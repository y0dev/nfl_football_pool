...
async function testEmail() {
  ...
  const requiredVars = ['NEXT_PUBLIC_SMTP_HOST', 'NEXT_PUBLIC_SMTP_PORT', 'NEXT_PUBLIC_SMTP_USER', 'NEXT_PUBLIC_SMTP_PASS', 'NEXT_PUBLIC_SMTP_FROM'];
  ...
  console.log(`Host: ${process.env.NEXT_PUBLIC_SMTP_HOST}`);
  console.log(`Port: ${process.env.NEXT_PUBLIC_SMTP_PORT}`);
  console.log(`User: ${process.env.NEXT_PUBLIC_SMTP_USER}`);
  ...
  const transporter = nodemailer.createTransporter({
    ...
    secure: process.env.NEXT_PUBLIC_SMTP_PORT === '465', // true for 465, false for other ports
    auth: {
  ...
  try {
    ...
    const testEmail = {
      ...
      to: process.env.NEXT_PUBLIC_SMTP_USER, // Send to yourself for testing
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background-color: #f8fafc; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            <p><strong>From:</strong> ${process.env.NEXT_PUBLIC_SMTP_FROM}</p>
            <p><strong>To:</strong> ${process.env.NEXT_PUBLIC_SMTP_USER}</p>
          </div>
