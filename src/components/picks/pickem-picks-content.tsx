...
import { PickemUserSelection } from '@/components/picks/pickem-user-selection';
    ...
    <div style={{ background: bg, minHeight: '100vh' }}>
      ...
      <PoolPicksHero
        ...
        actions={[
          ...
          // Once all participants are complete for the week, "Make Picks" is
          // relabeled "Show Leaderboard" (title only — same onClick, same
          // toggle target) since there's nothing left to pick.
          ...(!(effectiveGamesStarted || weekEnded) ? [{ label: showGameDetails ? (statsTotal > 0 && statsComplete >= statsTotal ? 'Show Leaderboard' : 'Make Picks') : 'Game Details', icon: showGameDetails ? EyeOff : Eye, onClick: () => setShowGameDetails(!showGameDetails) }] : []),
      ...
      {showGameDetails && (result?.eligibleGames.length ?? 0) > 0 && !(effectiveGamesStarted || weekEnded) && (
        <section style={{ background: bg, padding: '1.5rem 0 0' }}>
          <div className="lp-inner">
            <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 10, overflow: 'hidden' }}>
              <div style={{ padding: '1rem 1.25rem', borderBottom: `1px solid ${border}`, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <Target style={{ width: 15, height: 15, color: textMid }} />
                <span style={{ ...bc, fontWeight: 800, fontSize: '0.9rem', color: text, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{getWeekTitle(week ?? 1, seasonType ?? 2)} Game Details</span>
              </div>
              <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                {(result?.eligibleGames ?? []).map((game, index) => {
        ...
        <section style={{ background: surface, padding: '2rem 0' }}>
          <div className="lp-inner">
            <PickemUserSelection
              ...
              totalParticipantCount={result?.participants.length ?? 0}
              // forcePicks (dev only, gated behind showDebugPanel()) lists
              // everyone regardless of completion — lets a developer preview
              // an already-submitted participant's locked view. Never
              // reachable in production, where showDebugPanel() is false.
              availableParticipants={(showDebugPanel() && forcePicks
                ? (result?.participants ?? [])
                : (result?.participants ?? []).filter(p => !p.isComplete)
