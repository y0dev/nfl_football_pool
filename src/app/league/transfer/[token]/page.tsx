...
import { confirmHuddleTransfer } from '@/actions/huddleTransfers';
...
type ViewState =
  ...
  | { kind: 'waiting'; huddleName: string; otherEmail: string }
  | { kind: 'completed'; huddleName: string };
...
function TransferConfirmContent() {
  ...
  useEffect(() => {
    ...
    confirmHuddleTransfer(token, user.email).then(result => {
      if (!result.success) {
        setState({ kind: 'error', message: result.error });
      } else if (result.status === 'completed') {
        setState({ kind: 'completed', huddleName: result.huddleName });
      } else {
        setState({ kind: 'waiting', huddleName: result.huddleName, otherEmail: result.otherEmail });
      }
    });
  }, [token, user?.email]);
  ...
  const wrap = (icon: React.ReactNode, color: string, title: string, body: string) => (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: bg, padding: '2rem' }}>
      <div style={{ textAlign: 'center', maxWidth: 420, background: card, border: `1px solid ${border}`, borderTop: `3px solid ${color}`, borderRadius: 10, padding: '2rem' }}>
  ...
  if (state.kind === 'completed') {
    return wrap(
      ...
      green,
      'Transfer Complete',
      `"${state.huddleName}" has finished transferring. Both parties have been emailed a confirmation.`
    );
  }
  ...
  return wrap(
    ...
    `You've confirmed the transfer of "${state.huddleName}". It'll complete once ${state.otherEmail} confirms their side too.`
  );
}
