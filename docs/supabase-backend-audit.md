...
| Name | Purpose | Trigger | Dependencies | Auth |
|---|---|---|---|---|
| `determine-weekly-winners` | Computes and records weekly/period(Q1-Q4)/season winners per active pool from `picks` + `games.winner`, writing `scores`/`weekly_winners`/`period_winners`/`season_winners` | HTTP POST (manual, or the pg_cron schedule below once installed) | `jsr:@supabase/functions-js`, `jsr:@supabase/server@^1` (`withSupabase`) | `auth: 'secret'` — requires a real secret (service-role) key on the `apikey` header |
| `update-game-scores` | Refreshes `status`/`home_score`/`away_score`/`winner` on `games` from ESPN's public scoreboard API for games not yet finished | HTTP POST (manual, or the pg_cron schedule below) | `jsr:@supabase/functions-js`, `jsr:@supabase/server@^1`, ESPN public scoreboard API (no API key required) | `auth: 'secret'` |

Both functions were migrated this session from the older `deno.land/std` `serve()` + manual `createClient(Deno.env.get('SUPABASE_URL'), ...)` pattern to `jsr:@supabase/server`'s `withSupabase()` wrapper (matching Supabase's current dashboard-scaffold style — see §7). `ctx.supabaseAdmin` (an RLS-bypassing client pre-configured by the wrapper) replaces the manual `createClient()` call; `SUPABASE_URL`/`SUPABASE_JWKS`/key env vars are read by the wrapper itself, not by this code. **Real, deliberate security improvement, not just a style change**: the old code never checked caller identity at all — any request that passed Supabase's platform-level JWT check (the default) reached the function and got a full service-role client regardless of who sent it. `auth: 'secret'` now requires the caller to present the project's actual secret/service-role key — a genuinely narrower, correct set of callers (pg_cron, another Edge Function, or a manual admin test) than before.

This requires each function's `verify_jwt` deploy setting to be `false` (new `supabase/config.toml`, `[functions.<name>] verify_jwt = false`) — otherwise the platform's own JWT gate would reject a secret-key caller before `withSupabase`'s own check ever runs, since a secret key isn't a JWT. The CLI picks this up automatically on `supabase functions deploy <name>` once the project is linked; it isn't applied just by the file existing in the repo.

**Deployment status**: unverifiable from this environment (no CLI auth). Both functions' source is current in this repo as of this commit. To verify live deployment matches this source: Dashboard → Edge Functions → check "Last deployed" timestamp against this repo's latest commit touching `supabase/functions/`, or run `npx supabase functions deploy <name> --project-ref muvtenjtdzlwcwmzksxy` (after `supabase login`) to force-redeploy current source (this will also apply the new `verify_jwt = false` setting from `supabase/config.toml`).
...
**Unverifiable from this environment** (no CLI auth, no dashboard access) — see the access-constraints note at the top of this doc. What *is* confirmed: the app's own runtime env vars needed for the Next.js side (`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_SERVICE_KEY`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `API_SPORTS_KEY`, SMTP vars) are present in `.env.local` and functioning (used throughout this audit to query live data, and confirmed working by the existing E2E test baseline). Edge Function auth/config (`SUPABASE_URL`, `SUPABASE_JWKS`, `SUPABASE_PUBLISHABLE_KEYS`, `SUPABASE_SECRET_KEYS` — read internally by `jsr:@supabase/server`'s `withSupabase()` wrapper as of this session's migration, see §1) is Supabase-managed and auto-injected — not something to configure by hand, but its presence/correctness in the deployed function can only be confirmed via `npx supabase secrets list` (needs CLI auth) or the Dashboard. The `service_role_key` Vault secret used by the pg_cron migration to authenticate as a "secret" caller is the same value as this project's secret API key — substitute it once when running that migration (§1).
...
**Follow-up, now done**: Supabase's current dashboard "Create a new function" scaffold defaults to a newer runtime API (`jsr:@supabase/server`'s `withSupabase` helper + `jsr:@supabase/functions-js/edge-runtime.d.ts`), different from the `deno.land/std` `serve()` + manual `createClient()` pattern this repo's two functions originally used. Both functions were migrated to the newer style this session (see §1) — both now use `withSupabase({ auth: 'secret' })`.

**Corrected, single-line, copy-paste-safe test command** — updated for the migrated functions' auth: they now check the secret key on the `apikey` header, not `Authorization: Bearer` (replace `YOUR_REAL_SECRET_KEY` with this project's actual secret/service-role key; the function ignores its request body entirely so no `--data` is needed):

```
curl -X POST "https://muvtenjtdzlwcwmzksxy.supabase.co/functions/v1/determine-weekly-winners" -H "apikey: YOUR_REAL_SECRET_KEY" -H "Content-Type: application/json"
```

Run this (in any shell — it has no line continuations to break) to confirm the hostname issue is resolved. If it still fails the same way, that would rule out the shell-pasting theory and point to something else (e.g. a DNS/network issue in that specific environment) — worth re-investigating with the exact error output at that point. Note this corrected command only works once the function is redeployed with the migrated code and `verify_jwt = false` (§1) — until then, the previously-deployed version (if any) still expects whatever auth its old code path used.
...
- **Environment-variable validation**: originally both functions used a non-null assertion (`Deno.env.get('SUPABASE_URL')!`) with no explicit check, and a manual check was added for that. That manual check became moot once both functions were migrated to `withSupabase()` (§1) — the wrapper reads `SUPABASE_URL`/`SUPABASE_JWKS`/key env vars itself and fails clearly before the handler runs if they're misconfigured, so no redundant check is needed in this code anymore.
